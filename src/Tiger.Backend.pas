...
type
  ...
  TTigerBackend = class;
  TTigerDataBuilder = class;
  TTigerImportBuilder = class;
  TTigerExportBuilder = class;
  ...
  public
    constructor Create(const ABackend: TTigerBackend); reintroduce;
...
constructor TTigerDataBuilder.Create(const ABackend: TTigerBackend);
begin
  inherited Create();
...
constructor TTigerImportBuilder.Create(const ABackend: TTigerBackend);
begin
  inherited Create();
...
constructor TTigerExportBuilder.Create(const ABackend: TTigerBackend);
begin
  inherited Create();
...
begin
  LFunc := FBackend.GetCode.GetFunc(AFuncHandle.Index);
...
constructor TTigerCodeBuilder.Create(const ABackend: TTigerBackend);
begin
  inherited Create();
...
begin
  ...
  FData := TTigerDataBuilder.Create(Self);
  FGlobals := TTigerDataBuilder.Create(Self);
  FImports := TTigerImportBuilder.Create(Self);
  FExports := TTigerExportBuilder.Create(Self);
  FCode := TTigerCodeBuilder.Create(Self);
...
begin
  ...
  try
    Result := TUtils.RunExe(FOutputPath, '', ExtractFilePath(FOutputPath));
  except
    on E: Exception do
    begin
...
function TTigerBackend.GeneratePE(): TBytes;
const
...
begin
  ...
  try
    ...
    if FData.GetSize() > 0 then
      LRDataSection.WriteBuffer(FData.GetData()[0], FData.GetSize());
    ...
    if FGlobals.GetSize() > 0 then
      LDataSection.WriteBuffer(FGlobals.GetData()[0], FGlobals.GetSize());
    // Ensure section has at least 1 byte (PE requires non-zero SizeOfRawData)
    ...
    begin
      ...
      // - 32 bytes shadow space (Win64 ABI)
      // - 8 bytes per parameter (plus 1 for hidden count in variadic)
      // - Sum of local sizes (each aligned to 8)
      // - 8 bytes per temp
      // - Aligned to 16 bytes (accounting for pushed RBP)
      LLocalsSize := 0;
      for LJ := 0 to High(LFunc.Locals) do
        LLocalsSize := LLocalsSize + LFunc.Locals[LJ].LocalSize;
      if LFunc.IsVariadic then
        LStackFrameSize := 32 + Cardinal(4 * 8) + Cardinal(LLocalsSize) + Cardinal(LFunc.TempCount * 8)  // Always 4 slots for variadic
      else
        LStackFrameSize := 32 + Cardinal(Length(LFunc.Params) * 8) + Cardinal(LLocalsSize) + Cardinal(LFunc.TempCount * 8);
...
function TTigerBackend.GenerateCOFF(): TBytes;
const
  // x64 register encodings (same as GeneratePE)
  REG_RAX = 0; REG_RCX = 1; REG_RDX = 2; REG_RBX = 3;
  REG_RSP = 4; REG_RBP = 5; REG_RSI = 6; REG_RDI = 7;
  REG_R8 = 8; REG_R9 = 9; REG_R10 = 10; REG_R11 = 11;
  ...
  try
    ...
    begin
      ...
      for LJ := 0 to High(LFunc.Locals) do
        LLocalsSize := LLocalsSize + LFunc.Locals[LJ].LocalSize;
      if LFunc.IsVariadic then
        LStackFrameSize := 32 + Cardinal(4 * 8) + Cardinal(LLocalsSize) + Cardinal(LFunc.TempCount * 8)
      else
        LStackFrameSize := 32 + Cardinal(Length(LFunc.Params) * 8) + Cardinal(LLocalsSize) + Cardinal(LFunc.TempCount * 8);
      if (LStackFrameSize mod 16) <> 0 then
...
function TTigerBackend.GenerateLib(): TBytes;
var
  LObjData: TBytes;
  LOutput: TMemoryStream;
  LFunc: TTigerFuncInfo;
