...
type
  ...
  // TTigerRuntime
  //============================================================================
  // Runtime library that adds standard routines to the IR.
  // These routines get compiled into the final binary like user code.
  //
  // All runtime functions use the Tiger_ prefix to avoid name collisions with
  // user code and imported C libraries.
  //============================================================================

  TTigerRuntime = class
  public
    // System essentials (ExitProcess, Tiger_Halt)
    class procedure AddSystem(const AIR: TTigerIR);

    // I/O (tiger_write, tiger_writeln)
    class procedure AddIO(const AIR: TTigerIR);

    // Memory management (Tiger_GetMem, Tiger_FreeMem, etc.)
    class procedure AddMemory(const AIR: TTigerIR; const AOptLevel: Integer);

    // String management (Tiger_StrAlloc, Tiger_StrFree, etc.)
    class procedure AddStrings(const AIR: TTigerIR);

    // Exception handling (Tiger_Raise, Tiger_GetExceptionCode, etc.)
    class procedure AddExceptions(const AIR: TTigerIR);

    // Add all runtime support
    class procedure AddAll(const AIR: TTigerIR; const AOptLevel: Integer);
  end;
...
class procedure TTigerRuntime.AddSystem(const AIR: TTigerIR);
begin
  //----------------------------------------------------------------------------
  ...
  AIR.BeginFunc('Tiger_Halt', vtVoid, False, plC, False)
     ...
     .Call('ExitProcess', [AIR.Var_('AExitCode')])
  .EndFunc();
end;
...
class procedure TTigerRuntime.AddIO(const AIR: TTigerIR);
begin
  //----------------------------------------------------------------------------
...
class procedure TTigerRuntime.AddMemory(const AIR: TTigerIR; const AOptLevel: Integer);
begin
  //----------------------------------------------------------------------------
...
class procedure TTigerRuntime.AddStrings(const AIR: TTigerIR);
var
  LDeref: TTigerIRExpr;
begin
  //----------------------------------------------------------------------------
...
class procedure TTigerRuntime.AddExceptions(const AIR: TTigerIR);
begin
  //----------------------------------------------------------------------------
...
class procedure TTigerRuntime.AddAll(const AIR: TTigerIR; const AOptLevel: Integer);
begin
  AddSystem(AIR);
  AddIO(AIR);
